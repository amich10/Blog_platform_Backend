    ...
    transformCreatepost = async(req) =>{
        try {
            ...
            if (payload.status === postStatus.PUBLISHED) {
                ...
                payload.publishedAt = null; // Leave as null if not published
            }
             // Handle image upload (multiple images)
        if (req.files && req.files.length > 0) {
            payload.images = [];
            for (let file of req.files) {
                // Assuming cloudinarySvc.fileUpload() returns both the original and optimized image URLs
                let uploadedImage = await cloudinarySvc.fileUpload(file.path, '/post');
                payload.images.push({
                    url: uploadedImage.url,
                    optimizedUrl: uploadedImage.optimizedUrl // Assuming optimizedUrl is returned
                });
            }
        }
            return payload
