...
import { categoryDTO, updateCategoryDTO } from "./posts.vallidator.js";
import categoryCtrl from "./category.controller.js";

const postRouter = Router()

PostRouter.post('/create',allowUsers(userRoles.ADMIN),uploader().single('image'),bodyValidator(categoryDTO),categoryCtrl.storeCategory)
...
categoryRouter.patch('/update/:id',allowUsers(userRoles.ADMIN),uploader().single('image'),bodyValidator(updateCategoryDTO),categoryCtrl.updateCategoryById)
categoryRouter.delete('/delete/:id',allowUsers(userRoles.ADMIN),categoryCtrl.deleteCatgoryById)
