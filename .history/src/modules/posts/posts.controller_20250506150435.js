...
class postControl {
  ...
  // ✅ Sync comments count manually based on actual number of comments
  syncCommentCount = async (req, res, next) => {
    try {
      const postId = req.params.id;

      const commentCount = await Commen.countDocuments({ postId });
      ...
      res.json({
        message: "Comment count synced successfully",
        status: "COMMENT_COUNT_SYNCED",
        data: {
          postId: post._id,
