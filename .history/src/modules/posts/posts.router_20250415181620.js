...
postRouter.post('/create',allowUsers(userRoles.ADMIN),uploader().single('image'),bodyValidator(postDTO),postCtrl.storepost)
postRouter.get('/all',allowUsers(),postCtrl.showAllpost)
postRouter.get('/:id',allowUsers(),postCtrl.getpostDetailById)
postRouter.get('/all/published-categories',allowUsers(),postCtrl.getPublishedCategories)
