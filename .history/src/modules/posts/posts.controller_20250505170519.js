...
class postControl {
    ...
    incrementViews = async(req,res,next) =>{
        try {
            ...
            // Check if the post has been viewed before
    const lastViewed = post.lastViewedAt || 0; // Default to 0 if no lastViewedAt exists
    const currentTime = Date.now();
    
    // If the post was viewed less than a minute ago, don't set the timeout again
    if (currentTime - lastViewed < 60000) {
      return res.status(200).json({
        message: "Already queued for view count increment",
        views: post.views,
      });
    }
