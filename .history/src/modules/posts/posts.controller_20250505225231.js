...
class postControl {
  ...
  try {
    ...
    // Get or initialize the view tracking map
    if (!global.viewTracker) {
      global.viewTracker = {};
    }

    const key = `${slug}-${clientIp}`;
    const lastViewedAt = global.viewTracker[key] || 0;

    // 1-minute throttle per IP
    if (currentTime - lastViewedAt < 60000) {
      return res.json({
        message: "View already counted recently. Please wait before refreshing again.",
        views: post.views,
      });
    }

    // Update view tracker and increment view count
    global.viewTracker[key] = currentTime;
    post.views += 1;
    await post.save();

    return res.status(200).json({
      message: "Thanks for viewing! View count updated.",
      views: post.views,
    });
