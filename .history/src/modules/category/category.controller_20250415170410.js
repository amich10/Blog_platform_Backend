...
class CategoryControl {
   ...
   const getPublishedCategories = async (req, res, next) => {
    try {
        ...
        // Fetch categories using the service with the corrected filter
        const { data, pagination } = await categorySvc.listAllCategory({
            filter,
            query: req.query
        });

        // Return the response with the categories and pagination details
        res.json({
            data: data,
            message: "All active/published categories",
