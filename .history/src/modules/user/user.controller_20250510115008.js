...
getUserPosts = async (req, res, next) => {
  try {
    const userId = req.params.userId; // assuming it's passed as a route param

    const posts = await PostModel.find({ authorId: userId })
      .populate("categoryId") // optional: populate category if used
      .populate("authorId", "fullName username image") // show limited user info
      .sort({ createdAt: -1 });
