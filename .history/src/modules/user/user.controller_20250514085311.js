  ...
  createUser = async (req, res, next) => {
    try {
      ...
      const uploadedImages = await Promise.all(
        req.files.map((file) => cloudinarySvc.fileUpload(file.path, "users/"))
      );
    ...
    try {
      ...
      if (payload.username && payload.username !== user.username) {
       ...
       if (req.files?.image?.[0]) {
        const uploadedImage = await cloudinarySvc.fileUpload(req.files.image[0].path, "users/");
        payload.image = uploadedImage;
      }

      // Upload new cover image if provided
      if (req.files?.coverImage?.[0]) {
        const uploadedCoverImage = await cloudinarySvc.fileUpload(req.files.coverImage[0].path, "users/covers/");
