...
class UserController {
  createUser = async (req, res, next) => {
    try {
      ...
      payload.coverImage = await cloudinarySvc.fileUpload(req.file.path,'users/cover/')
  ...
  if (req.files.image && req.files.image[0]) {
    payload.image = await cloudinarySvc.fileUpload(req.files.image[0].path, "users/");
  }

  if (req.files.coverImage && req.files.coverImage[0]) {
    payload.coverImage = await cloudinarySvc.fileUpload(req.files.coverImage[0].path, "users/cover/");
  }
}
