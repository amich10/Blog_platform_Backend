...
class AuthService {
  ...
  emailActivationNotify = async ({
    ...
    try {
      const activationLink = `${appConfig.frontendUrl}/auth/activate/${activationToken}`;
  ...
  notifyForgetPasswordEmail = 
  async ({name,email,forgetPaswordToken}) =>{
      try{
          ...
          <a href="${appConfig.frontendUrl}verify-frget-token/${forgetPaswordToken}" style="color: #008000; text-decoration:underline;">
          ${appConfig.frontendUrl}verify-forget-token/${forgetPaswordToken}</a>
          <br>
